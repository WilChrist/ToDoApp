  ...
  case msg of
    Change newSentence->
      (
        ...
        --in
        {model  | sentenceInput=newSentence}, Cmd.none)
    Create ->
      (
        --let 
    ...
    Delete idToDelete->
      (
        ...
        --let 
          --_=Debug.log "D=" model
        --in
        { model | todos = deleteFromCollection idToDelete model.todos }, Cmd.none)
    DeleteAll ->
      (
        --let 
          --_=Debug.log "DQ=" model
        --in
        {model | todos = deleteAllFromCollection model.todos}, Cmd.none)
...
view model =
  div [class "wrapper"]
    ...
    , div [class "inputField"]
        ...
        , button [onClick (Create), if (String.length model.sentenceInput)>0 then class "active" else class ""]
            [i [class "fas fa-plus"][]]
        ]
